...
void TypeImpl::SetName(const std::string_view& name) { m_name = name.data(); }
...
size_t TypeImpl::GetInstanceSize() const { return m_instanceSize; }
...
void TypeImpl::SetFlags(TypeFlag flags) { m_flags = flags; }

cppreflection::TypeFlag TypeImpl::GetFlags() const { return m_flags; }
...
void TypeImpl::SetComparisonOperators(
    const TypeComparisonOperators& comparisonOperators) {
  m_comparisonOperators = comparisonOperators;
}

const cppreflection::TypeComparisonOperators& TypeImpl::GetComparisonOperators()
    const {
  return m_comparisonOperators;
}

std::span<const Field* const> TypeImpl::GetFields() const { return m_fields; }

std::span<const Function* const> TypeImpl::GetMethods() const {
  return m_methods;
}

const edt::GUID& TypeImpl::GetGuid() const { return m_guid; }

void TypeImpl::SetGUID(const edt::GUID& guid) { m_guid = guid; }
...
CPP_REFLECTION_API Type* AllocType() { return new detail::TypeImpl(); }
}  // namespace cppreflection
